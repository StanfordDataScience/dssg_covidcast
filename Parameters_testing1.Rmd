---
title: Parameterized report_michelle
output: 
  html_document:
    code_folding: hide
params:
  forecasters:
    ...
    value: [COVIDhub-ensemble, COVIDhub-baseline]
    input: select
    multiple: True
  weeks: 
    label: "Weeks"
...
#unique(state_predictions$signal)
```
...
submitted <- lapply(competition[1:length(params$forecasters)], get_covidhub_predictions,
                    forecast_dates = forecast_dates,
                    signal = "deaths_incidence_num",
                    ahead = aheads)
...
ggplotly(plot_wis, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
...
#  geom_hline(yintercept = .8, color="black")+
    geom_point(aes(text=sprintf("Forecast Date: %s<br>Mean WIS: %s <br>Forecaster: %s", forecast_date, round(coverage_80, digits =3),color)),alpha = 0.05)+
  theme_bw() +
  facet_wrap(~ahead, nrow = 4, labeller = labeller(ahead=facet.label))
  

ggplotly(plot_cover, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
...
ggplotly(median_wis_forecast_date, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
```
...
ggplotly(mean_wis, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
...
ggplotly(mean_wis_forecast_date, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
```
...
ggplotly(target_wis_ahead, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
