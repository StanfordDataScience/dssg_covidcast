...
params:
  ...
  states:
    ...
    value: ak
    input: select
    choices: [ak,al, ar, az, ca, co, ct, dc, de, fl, ga, hi, ia, id, il, in, ks, ky, la, ma, md, me, mi, mn, mo, ms, mt, nc, nd, ne, nh, nj, nm, nv, ny, oh, ok, or, pa, pr, ri, sc, sd, tn, tx, ut, va, vt, wa, wi, wv, wy]
    multiple: True
  weeks: 
    label: "Weeks"
    value: 1
...
knitr::opts_chunk$set(autodep = TRUE, fig.align = "center", fig.width = 12, message = FALSE, warning = FALSE, echo=params$printcode)
...
```{r state-predictions, cache=TRUE, results="hide", message=FALSE, warning=FALSE}
...
```{r grab-the-competition, cache=TRUE}
...
plot_wis = plot_canonical(results, x = "forecast_date", y = "wis", aggr = mean,
 ...
 coord_cartesian(ylim=c(0,50)) +
    geom_point(aes(text=sprintf("Weeks Ahead: %s<br>Mean WIS: %s <br>Forecaster: %s", ahead, round(wis, digits =3),color)),alpha = 0.05)+
  theme_bw(14)

ggplotly(plot_wis, tooltip="text") %>% layout(legend = list(orientation = "h", x = -0.25, y =0), hoverlabel=list(bgcolor='white'))
...
#  geom_hline(yintercept = .8, color="black")+
  geom_point(aes(text=sprintf("Weeks Ahead: %s<br>Coverage_80: %s <br>Forecaster: %s", ahead, round(coverage_80, digits =3),color)),alpha = 0.05)+
  theme_bw(14)
...
wis_ahead = plot_canonical(results, x = "ahead", y = "wis", aggr = median,
...
wis_forecast_date = plot_canonical(results, x = "forecast_date", y = "wis", aggr = median,
               grp_vars = c("forecaster", "ahead"), facet_rows = "ahead",
               base_forecaster = "COVIDhub-baseline", scale_before_aggr = TRUE) +
