...
params:
  forecasters:
    ...
    value: [COVIDhub-ensemble, COVIDhub-baseline, CMU-TimeSeries, Karlen-pypm, CU-select]
    input: select
    multiple: True
  weeks: 
    label: "Weeks"
...
#                  "CMU-TimeSeries", "Karlen-pypm","CU-select", "Google_Havard-CPF", "MIT-Cassandra")
...
#  coord_cartesian(ylim=c(0,100))+
  geom_point(aes(text=sprintf("Weeks Ahead: %s<br>Average Error: %s <br>Forecaster: %s", ahead, round(ae, digits =3),color)),alpha = 0.05)+
  theme_minimal()

ggplotly(plot_ae, tooltip="text", width=1200) %>% layout(hoverlabel=list(bgcolor='white'))

plot_wis = plot_canonical(results, x = "forecast_date", y = "wis", aggr = mean,
               grp_vars = c("forecaster","ahead"), facet_rows = "ahead") + 
  labs(title = subtitle, x= "Forecast Dates", y = "Mean WIS") +
 coord_cartesian(ylim=c(0,50)) +
    geom_point(aes(text=sprintf("Forecast Date: %s<br>Mean WIS: %s <br>Forecaster: %s", forecast_date, round(wis, digits =3),color)),alpha = 0.05)+
  facet_wrap(~ahead, nrow = 4, labeller = labeller(ahead=facet.label)) +
  theme_bw()+
    theme(strip.background = element_rect(fill = "white"))

ggplotly(plot_wis, tooltip="text", height=800, width= 1200) %>% layout(hoverlabel=list(bgcolor='white'))

plot_cover = plot_canonical(results, x = "forecast_date", y = "coverage_80", aggr = mean,
               grp_vars = c("forecaster","ahead"), facet_rows = "ahead") +
  labs(title = subtitle, x= "Forecast date", y = "Mean Coverage") +
#  coord_cartesian(ylim=c(0,1)) + 
#  geom_hline(yintercept = .8, color="black")+
    geom_point(aes(text=sprintf("Forecast Date: %s<br>Coverage: %s <br>Forecaster: %s", forecast_date, round(coverage_80, digits =3),color)),alpha = 0.05)+
  facet_wrap(~ahead, nrow = 4, labeller = labeller(ahead=facet.label)) +
  theme_bw()+
  theme(strip.background = element_rect(fill = "white"))

ggplotly(plot_cover, tooltip="text", height=800, width=1200) %>% layout(hoverlabel=list(bgcolor='white'))
...
median_wis <- plot_canonical(results, x = "ahead", y = "wis", aggr = median,
  ...
  labs(title = subtitle, x= "Weeks Ahead", y = "Median Relative WIS") +
  theme(legend.position = "bottom") + 
  geom_point(aes(text=sprintf("Weeks Ahead: %s<br>WIS: %s <br>Forecaster: %s", ahead, round(wis, digits =3),color)),alpha = 0.05)+
  theme_minimal()

ggplotly(median_wis, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))

median_wis_forecast_date <- plot_canonical(results, x = "forecast_date", y = "wis", aggr = median,
               grp_vars = c("forecaster", "ahead"), facet_rows = "ahead",
               base_forecaster = "COVIDhub-baseline", scale_before_aggr = TRUE) +
  labs(title = subtitle, x = "Forecast date", y = "Median relative WIS") +
  geom_point(aes(text=sprintf("Forecast Date: %s<br>WIS: %s <br>Forecaster: %s", forecast_date, round(wis, digits=3),color)),alpha = 0.05)+
  theme_bw() +
  facet_wrap(~ahead, nrow = 4, labeller = labeller(ahead=facet.label))+
    theme(strip.background = element_rect(fill = "white"))


ggplotly(median_wis_forecast_date, tooltip="text", height=800,width= 1200) %>% layout(hoverlabel=list(bgcolor='white'))
```
...
mean_wis = 
  plot_canonical(results %>% filter(wis > 0), x = "ahead", y = "wis", 
               ...
               base_forecaster = "COVIDhub-baseline", scale_before_aggr = TRUE) + 
  labs(title = subtitle, 
       x = "Weeks Ahead", y = "Mean (geometric) relative WIS") +
  theme(legend.position = "bottom") + 
  geom_point(aes(text=sprintf("Weeks Ahead: %s<br>WIS: %s <br>Forecaster: %s", ahead, round(wis, digits =3),color)),alpha = 0.05)+
  theme_minimal()

ggplotly(mean_wis, tooltip="text", width= 1200) %>% layout(hoverlabel=list(bgcolor='white'))
...
plot_canonical(results %>% filter(wis > 0), x = "forecast_date", y = "wis", 
  ...
  theme(legend.position = "bottom") + 
  labs(title = subtitle, 
       x = "Forecast date", y = "Mean (geometric) relative WIS") +
  geom_point(aes(text=sprintf("Forecast Date: %s<br>WIS: %s <br>Forecaster: %s", forecast_date, round(wis, digits =3),color)),alpha = 0.05)+
  theme_bw() +
  facet_wrap(~ahead, nrow = 4, labeller = labeller(ahead=facet.label))+
  theme(strip.background = element_rect(fill = "white"))

ggplotly(mean_wis_forecast_date, tooltip="text", height=800, width= 1200) %>% layout(hoverlabel=list(bgcolor='white'))
```
...
target_wis =
  ...
  labs(title = subtitle, x = "Target date", y = "Mean WIS") +
  scale_y_log10() +
  geom_point(aes(text=sprintf("Target End Date: %s<br>WIS: %s <br>Forecaster: %s", target_end_date, round(wis, digits =3),color)),alpha = 0.05)+
  theme_minimal()+
...
ggplotly(target_wis, tooltip="text") %>% layout(hoverlabel=list(bgcolor='white'))
ggplotly(target_wis_ahead, tooltip="text", height=800, width= 1200) %>% layout(hoverlabel=list(bgcolor='white'))
#table(submitted$forecaster)
#table(submitted$ahead)
```
...
g <- g +
  ...
  geom_line(data = pd$points_df, 
            mapping = aes(y = value, color = forecaster, group = interaction(forecaster, forecast_date)), size = .5) +
...
ggplotly(states, height=2500, width= 1200)
```
