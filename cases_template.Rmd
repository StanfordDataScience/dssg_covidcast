---
title: Cases Parameterized report
output: 
  html_document:
    code_folding: hide
params:
  forecasters:
    ...
    value: [CovidAnalytics-DELPHI,COVIDhub-baseline, COVIDhub-ensemble,COVIDhub-trained_ensemble, CU-select, Google_Harvard-CPF,Karlen-pypm, MIT-Cassandra]
    input: select
    multiple: TRUE
  primary_forecaster:
...
results %>%
  download_this(
    ...
    button_label = "Download Cases Evaluation Dataset",
    button_type = "success",
    has_icon = TRUE,
    csv2 = FALSE,
    icon = "fa fa-save"
...
The plots are created based on COVIDhub-ensemble forecaster as the base forecaster. 
Relative to baseline; scale first then take the geometric mean, ignoring a few 0's.
```{r,height=800, width= 1000, message = FALSE, warning = FALSE}
